/* ************************************************************************* *\
*                Programming 1 HS 2016 - Exercise 3-1                        *
\* ************************************************************************* */
// Done By:
// Brian S. 16-xxx-yyy
// Julius O.  16-xxx-yyy
...
public class GuessOrc {
	...
	public int calculateHint( int guessedMineShaftId ) {
		...
		if ( java.lang.Math.abs(guessedMineShaftId - mineShaftId) > 5 ) {
			...
			if (guessedMineShaftId == mineShaftId) {
				// Orcs got hit, game won.
				return NO_HINT;
			} else {
				// to the left  /  to the right
				possibleHints.add((guessedMineShaftId > mineShaftId) ?  0 : 1);
			}
		}
	...
	public void start() {
		Random random = new Random();
		mineShaftId = random.nextInt(12) + 1;

		// Greeting user with Orc and printing current Map.
		System.out.println(this.generateOrcIntroScreen());
		System.out.println("");
	...
	public void gameplay() {
		...
		while (round <= MAX_ATTEMPTS) {
			// Guessing the column
			System.out.println("Guess a mineshaft: ");
			String guess = scan.nextLine().toLowerCase();
			char guessChar = guess.charAt(0);
			int helpId;

			if (Arrays.asList('a','b','c','d','e','f','g','h','i','j','k','l').contains(guessChar) && guess.length() ==1) {
				// input as int
				int guessInt = getColumnAsInt(guessChar);

				// Calculates the Hint for the User
				helpId = calculateHint(guessInt);

				//Prints the Map
				printMap(guessInt, helpId);
			...
			// if user input is illegal
				...
				System.out.println("Please choose a single valid mineshaft ID to flush!");
				round--;
			}
			round++;
		} // end of while{}
		...
		if (round > MAX_ATTEMPTS)
			System.out.println("You failed to defeat the orcs in shaft " + getMineShaftAsChar(mineShaftId) );
	}
